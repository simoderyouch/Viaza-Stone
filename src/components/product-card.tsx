...
export async function ProductCard({ product, compact = false }: { product: Product; compact?: boolean }) {
  ...
  return (
    <Link href={`/catalogue/${localizedProduct.slug}`} aria-label={`View ${localizedProduct.name}`} className="group block focus:outline-none">
      <article className="overflow-hidden border border-stone-200 bg-white transition duration-200 group-hover:-translate-y-1 group-hover:shadow-xl group-focus-visible:ring-2 group-focus-visible:ring-[#8d8067] group-focus-visible:ring-offset-2">
        ...
        <div className="p-5">
          ...
          {!compact && <p className="mt-2 truncate text-sm leading-6 text-stone-600">{localizedProduct.description}</p>}
          ...
          <span className="mt-4 inline-flex text-[0.66rem] font-bold tracking-[0.14em] text-[#282828] uppercase transition group-hover:text-stone-500">
