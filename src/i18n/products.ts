...
function translatedDescription(product: Product, locale: Exclude<Locale, 'en'>, finish: string) {
  if (locale === 'fr') {
    if (product.type === 'Blocs & Slabs') return `Pierre naturelle proposée en blocs et en tranches, à sélectionner selon le matériau, le format et les exigences du projet.`
  ...
  if (product.type === 'Blocs & Slabs') return 'حجر طبيعي متوفر على شكل كتل وألواح، يتم اختياره بحسب المادة والمقاس ومتطلبات المشروع.'
...
export function localizeProduct(product: Product, locale: Locale): Product {
  ...
  return {
    ...
    formats: product.formats.map((item) => isFrench ? ({ Tiles: 'Dalles', Blocks: 'Blocs', Slabs: 'Tranches', 'Wall cladding': 'Revêtement mural', 'Cut-to-size': 'Découpe sur mesure' }[item] ?? item) : ({ Tiles: 'بلاطات', Blocks: 'كتل', Slabs: 'ألواح', 'Wall cladding': 'كسوة جدارية', 'Cut-to-size': 'قص حسب المقاس' }[item] ?? item)),
